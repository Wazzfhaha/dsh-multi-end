...
test('adding an Agent Box workspace refreshes the native sidebar without touching the local backend', async () => {
  ...
  await model.workspace({ ...connection, target }, 'mkdir', '/home/test', undefined, 'new folder')
  const workspaceCalls = calls.filter(call => call.action === 'workspace')
  assert.equal(workspaceCalls.length, 2)
  assert.equal(workspaceCalls[1].input.name, 'new folder')
  assert.equal(workspaceCalls[1].input.action, 'mkdir')
